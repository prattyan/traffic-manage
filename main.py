...
def process_traffic():
    ...
    while cap.isOpened():
        ...
        cv2.putText(frame, f'Traffic Decision: {decision}', (50, 50),
...
def update_dashboard(n):
    ...
    fig = {
        "data": [
            {"x": timestamps, "y": vehicle_counts,
             "type": "line", "name": "Vehicle Count"}
        ],
        "layout": {"title": "Traffic Volume Over Time"}
    }

    return fig, f"Latest Decision: {decisions[-1]}"


if __name__ == "__main__":
    traffic_thread = threading.Thread(
        target=process_traffic,
        daemon=True
    )
    traffic_thread.start()

    # ✅ Updated Dash API (fixes startup crash)
